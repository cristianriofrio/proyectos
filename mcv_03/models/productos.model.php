...
class Producto
{
    public function todos() // select * from productos
    {
        ...
        LEFT JOIN `IVA` i ON p.Graba_IVA = i.idIVA
    ...
    public function uno($idProductos) // select * from productos where id = $idProductos
    {
        ...
        LEFT JOIN `Unidad_Medida` u ON k.Unidad_Medida_idUnidad_Medida = u.idUnidad_Medida
        LEFT JOIN `IVA` i ON p.Graba_IVA = i.idIVA
        where k.`Estado` = 1 and p.idProductos = $idProductos";
        $datos = mysqli_query($con, $cadena);
        $con->close();
        return $datos;
    }
    ...
    public function insertar($Codigo_Barras, $Nombre_Producto, $Graba_IVA, $Unidad_Medida_idUnidad_Medida, $IVA_idIVA, $Cantidad, $Valor_Compra, $Valor_Venta, $Proveedores_idProveedores)
    {
        try {
            ...
            if (mysqli_query($con, $cadenaProducto)) {
                ...
                $cadenaKardex = "INSERT INTO `Kardex`(`Estado`, `Fecha_Transaccion`, `Cantidad`, `Valor_Compra`, `Valor_Venta`, `Unidad_Medida_idUnidad_Medida`, `IVA`, `IVA_idIVA`, `Proveedores_idProveedores`, `Productos_idProductos`, `Tipo_Transaccion`)
                                 VALUES (1, NOW(), '$Cantidad', '$Valor_Compra', '$Valor_Venta', '$Unidad_Medida_idUnidad_Medida', '$IVA_idIVA', '$IVA_idIVA', '$Proveedores_idProveedores', '$productoId', 1)";
